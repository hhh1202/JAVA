...
public class ClientApp {
    ...
    public ClientApp() throws IOException {
        this.socket = new Socket(); // 클라이언트 통신용 소켓 생성
    }

    public void connect() throws IOException {
        this.socket.connect(new InetSocketAddress("localhost", 59999)); // 해당 ip와 포트로 접속을 시도한다.
//        this.socket.bind(new InetSocketAddress("localhost",5999)); // ip와 포트 정보를 bind
    }

    public void close() throws IOException {
        this.socket.close(); // 클라이언트 통신 소켓을 닫는다.
    }
